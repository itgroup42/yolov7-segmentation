...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None):
        ...
        exts = ['.jpg', '.jpeg', '.png']
        files = [str(p) for p in Path(path).rglob('*') if p.suffix in exts]
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            pad_side = int(0.5 * im0.shape[0] / 2.0)
            pad_over = int(0.5 * im0.shape[1] / 2.0)
            im0 = np.pad(im0, ((pad_side,pad_side),(pad_over,pad_over),(0,0)), 'constant')
        ...
        return path, im, im0, self.cap, s, (pad_side, pad_over)
